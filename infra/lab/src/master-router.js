...
const CORS_ALLOWED = new Set([
  ...
  "https://thawani.tail19ddab.ts.net",
  "http://localhost:3003",
]);
...
// 🎯 ترحيب مختصر (بلا جمل زائدة مثل "اضغط الرابط لفتح القائمة")
...
// ─── SSE: Real-time events للماستر ───────────────────────────────────────────
// يستقبل: store_created, subscription_changed, store_offline, wa_status
router.get("/master/events", (req, res) => {
  // يدعم header الأساسي + query (للوصول من EventSource في المتصفح)
  const token = req.headers["x-master-token"] || req.query.token;
  const ts = sessions.get(token);
  if (!token || ts === undefined) return res.status(401).end();
  if (Date.now() - ts > MASTER_SESSION_TTL_MS) return res.status(401).end();
  res.set({
  ...
  // نستخدم نفس global registry (sseAdd/Remove) من server.js لكن بـ storeId خاص "master"
  global.sseAdd?.("master", res);
  req.on("close", () => global.sseRemove?.("master", res));
});

// helper داخلي ينشر للماستر
function _mEmit(event, data) {
  try { global.sseSend?.("master", event, data); } catch {}
}
...
// ─── 🧠 NEXUS Watch — صيانة ذكية للمنصة ─────────────────────────────────────
const nexusWatch = require("./nexus-watch");

router.get("/master/nexus/stats", auth, (req, res) => {
  res.json(nexusWatch.getStats());
});

router.get("/master/nexus/agents", auth, (req, res) => {
  res.json({ agents: nexusWatch.getAgents() });
});

router.post("/master/nexus/health-digest", auth, async (req, res) => {
  try {
    const r = await nexusWatch.healthDigest();
    res.json(r);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post("/master/nexus/code-review", auth, async (req, res) => {
  try {
    const files = Array.isArray(req.body?.files) ? req.body.files.slice(0, 10) : [];
    const r = await nexusWatch.codeReview(files);
    res.json(r);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post("/master/nexus/insights/:storeId", auth, async (req, res) => {
  try {
    const r = await nexusWatch.businessInsights(req.params.storeId);
    res.json(r);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// 🌐 GET public endpoint للموقع التسويقي — يُعرض في landing page
// CORS مفعّل + لا auth (الباقات معلومات عامة للمستخدمين الجدد)
router.options("/api/public/plans", (req, res) => {
  setApiCors(req, res);
  res.sendStatus(204);
});
router.get("/api/public/plans", (req, res) => {
  setApiCors(req, res);
  const plans = readOwnerSettings().plans || {};
  // أرجِع فقط الحقول العامة (لا أسرار/internals)
  const publicPlans = {};
  for (const [id, p] of Object.entries(plans)) {
    publicPlans[id] = {
      ...
      featured: !!p.featured,
      badge: p.badge || "",
      displayFeatures: p.displayFeatures || p.feats || [],
      track: (p.track === "bundle" ? "bot" : p.track) || "bot",
    };
  }
  res.json({ plans: publicPlans });
});

// 🌐 GET public endpoint لرقم دعم المنصة (يُستخدم في زر "تواصل مع منصة ثواني")
router.get("/api/public/support-phone", (req, res) => {
  setApiCors(req, res);
  const settings = readOwnerSettings();
  const phone = String(settings.supportPhone || process.env.MASTER_PHONE || "").replace(/\D/g, "");
  res.json({ phone, hours: settings.supportHours || "" });
});
...
router.post("/master/stores", auth, async (req, res) => {
  ...
  // احفظ نسخة plain للماستر (للدعم) + اصنع hash للمصادقة
  const plain = String(store.storePassword || "").trim();
  if (plain && !BCRYPT_RE.test(plain)) {
    store.storePasswordPlain = plain;
    try { store.storePassword = await bcrypt.hash(plain, BCRYPT_ROUNDS); }
    catch (e) { return res.status(500).json({ error: "فشل تشفير كلمة المرور" }); }
  }

  data.stores.push(store);
  writeStores(data);

  // Sync to Firestore — مرّر plain (upsert يـ hash داخلياً)؛ تمرير hash كان يسبّب double-hash
...
const MASTER_STORE_FIELDS = new Set([
  ...
  "loyaltySettings",
  "trackOverride", // ⭐ override للمسار (cart/bot/null) — يتجاوز track الباقة
]);
...
router.put("/master/stores/:id", auth, async (req, res) => {
  ...
  // hash للـ storePassword لو أُرسل plaintext + احفظ plain للماستر
  if (typeof updates.storePassword === "string" && updates.storePassword.length > 0
      && !BCRYPT_RE.test(updates.storePassword)) {
    updates.storePasswordPlain = updates.storePassword;
    try {
  ...
  const prevTrackOv = data.stores[idx].trackOverride || null;
  data.stores[idx] = { ...data.stores[idx], ...updates, id: req.params.id };
  // null/empty string في trackOverride = إزالة الـ override
  if ("trackOverride" in updates && (updates.trackOverride === null || updates.trackOverride === "")) {
    delete data.stores[idx].trackOverride;
  }
  writeStores(data);

  // ⭐ إعلام لوحة الأدمن فوراً إن تغيّر المسار → تُعيد تحميل الـ tabs
  if ("trackOverride" in updates || ("plan" in updates && prevTrackOv !== updates.trackOverride)) {
    try {
      const { normalizeTrack } = require("./plans");
      const newTrack = data.stores[idx].trackOverride
        ? normalizeTrack(data.stores[idx].trackOverride)
        : normalizeTrack(require("./plans").getPlan(data.stores[idx].plan).track);
      global.sseSend?.(req.params.id, "track_changed", {
        track: newTrack,
        overridden: !!data.stores[idx].trackOverride,
  ...
  // Sync auth fields to Firestore — مرّر plain من req.body (upsert يـ hash داخلياً)
  const s = data.stores[idx];
  firestoreAuth.upsertStoreAdmin({
    storeId:            s.id,
    phone:              s.ownerPhone || "",
    ...(req.body.storePassword ? { password: String(req.body.storePassword) } : {}),
    storeName:          s.storeName || "",
    subscriptionStatus: s.subscriptionStatus || "active",
    active:             s.active !== false,
  }).catch(e => console.warn("Firestore sync error:", e.message));

  // عند تغيير الباس من الماستر: ألغِ كل tokens المتجر النشطة → يُجبر إعادة الدخول
  if (req.body.storePassword) {
    try {
      const revoked = global.revokeStoreTokens?.(s.id) || 0;
      if (revoked > 0) console.log(`🔓 [${s.id}] ${revoked} جلسة ادمن أُلغيت بسبب تغيير الباس من الماستر`);
    } catch (e) { console.warn("[master] revoke tokens failed:", e.message); }
  }

  // 🔇 إسكات البوت فوراً عند إيقاف/إلغاء الاشتراك أو تعطيل المتجر
  // disconnect Baileys session للمتجر المعطّل لمنع رد البوت على رسائل العملاء
  const justDeactivated = (prevStatus === "active" && s.subscriptionStatus !== "active")
                       || (req.body.active === false);
  if (justDeactivated) {
    try {
      // 1) أسكت البوت (Baileys socket)
      await waMgr.disconnectSession(s.id, { keepCreds: true });
      console.log(`🔇 [${s.id}] session disconnected — اشتراك متوقف/متجر معطّل (creds محفوظة)`);
    } catch (e) {
      console.warn(`[deactivate] disconnect failed for ${s.id}:`, e.message);
    }
    try {
      // 2) أغلق لوحة الادمن الخاصة بالمتجر (مسح كل tokens)
      const revoked = global.revokeStoreTokens?.(s.id) || 0;
      if (revoked > 0) console.log(`🔓 [${s.id}] أُلغيت ${revoked} جلسة ادمن`);
    } catch (e) {
      console.warn(`[deactivate] revoke tokens failed for ${s.id}:`, e.message);
    }
    _mEmit("subscription_changed", { storeId: s.id, storeName: s.storeName, status: s.subscriptionStatus, active: s.active !== false });
  }
  ...
  if (justActivated) {
    _mEmit("subscription_changed", { storeId: s.id, storeName: s.storeName, status: "active", active: true });
  }
...
// ─── Send store admin credentials to owner via WhatsApp ──────────────────────
router.post("/master/stores/:id/send-credentials", auth, async (req, res) => {
  const { stores } = readStores();
  const store = stores.find(s => s.id === req.params.id);
  if (!store) return res.status(404).json({ error: "المتجر غير موجود" });

  const phone = String(store.ownerPhone || "").replace(/[\s\+\-\(\)]/g, "");
  if (!phone) return res.status(400).json({ error: "رقم المالك غير محفوظ" });

  const plain = String(store.storePasswordPlain || "").trim();
  if (!plain) {
    return res.status(400).json({
      error: "كلمة المرور غير متاحة بصيغة plain — أعد تعيينها من نموذج التعديل أولاً"
    });
  }

  const PUBLIC_URL = (process.env.PUBLIC_URL || "").replace(/\/$/, "");
  const adminUrl   = `${PUBLIC_URL}/store-admin.html`;
  const storeName  = store.storeName || store.id;

  const msg =
`🎉 *مرحباً ${store.ownerName || ""}*
تم تجهيز لوحة تحكم متجرك *${storeName}* ✅
...
🔑 *بيانات الدخول:*
الرقم: ${store.ownerPhone}
كلمة المرور: ${plain}

━━━━━━━━━━━━━━━━━━━━━
💡 يمكنك تغيير كلمة المرور بعد الدخول من إعدادات المتجر.
💬 الدعم: wa.me/966508572902`;
  ...
  try {
    ...
    const activeSession = sessions.find(ss => ss.storeId === "platform" && ss.status === "open")
                       ...
                       || sessions.find(ss => ss.status === "open");
    if (!activeSession) {
      return res.status(503).json({ error: "لا توجد جلسة واتساب نشطة لإرسال الرسالة" });
    }
    await waMgr.sendMessage(activeSession.storeId, jid, msg);
    audit({
      actor: { type: "master", id: "master" },
      action: "store.credentials.sent",
      target: { type: "store", id: store.id },
      meta: { via: activeSession.storeId },
    }, req);
    res.json({ ok: true });
  } catch (e) {
...
router.post("/master/platform-bot/disconnect", auth, async (req, res) => {
  // 🛑 hard disconnect: أغلق socket + امسح creds + أوقف auto-reconnect
  try {
    await waMgr.disconnectSession?.("platform", { keepCreds: false });
  } catch (e) { console.warn("[platform-bot/disconnect]", e.message); }
  // fallback: تأكد أن المجلد فاضٍ (حتى لو الدالة فشلت)
  try {
    const fs   = require("fs");
    const path = require("path");
  ...
  res.json({ ok: true, message: "تم قطع الاتصال ومسح الجلسة نهائياً" });
});
...
router.post("/api/register-request", registerLimiter, async (req, res) => {
  setApiCors(req, res);
  const { name, phone, store, type, city, plan, track } = req.body || {};
  ...
  const cleanTrack = track === "bundle" ? "bot" : (["cart","bot"].includes(track) ? track : "bot");
...
نتشرف بخدمتك 🙏`;
  ...
  const TRACK_LABELS = { cart: "🛍️ سلة فقط", bot: "🤖 بوت فقط", bundle: "📦 متكامل (سلة+بوت)" };
  const trackName = TRACK_LABELS[cleanTrack] || cleanTrack;
  const masterMsg =
`📥 *طلب اشتراك جديد*
...
router.post("/master/approve-request/:id", auth, async (req, res) => {
  ...
  // ── AI-generated admin config + bot questions (يعملان في الخلفية بالتوازي) ──
  const _genBotQ = (() => {
    try { return require("./store-router").generateBotQuestionsForBiz; } catch { return null; }
  })();
  Promise.all([
    generateAdminConfig(pr.type).catch(e => { console.warn("[ai-admin] gen failed:", e.message); return null; }),
    _genBotQ
      ? _genBotQ(pr.type, pr.store).catch(e => { console.warn("[ai-botq] gen failed:", e.message); return null; })
    ...
    if (botQ && Array.isArray(botQ.fields) && botQ.fields.length) {
      data.stores[idx].botQuestions = {
        fields: botQ.fields,
        source: "ai_auto_register",
        updatedAt: new Date().toISOString(),
      };
      console.log(`[ai-botq] stored ${botQ.fields.length} questions for ${storeId} (${pr.type})`);
      changed = true;
    }
    if (changed) writeStores(data);
  });

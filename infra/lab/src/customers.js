...
// 🛡️ محمي من race conditions عبر atomicFs.withLock — يضمن read-modify-write سيريالي
function upsertCustomer({ phone, name, location, total, storeId, kind }) {
  return atomicFs.withLock(CUSTOMERS_PATH, () => {
    const db  = load();
    ...
    if (kind === "booking") {
      db[key].bookingsCount = (db[key].bookingsCount || 0) + 1;
      db[key].lastBooking = now;
    } else {
    ...
    save(db);
  }).catch(e => console.warn("[upsertCustomer] failed:", e.message));
}

...
### نشاط: "تأجير شاليهات / منازل / استراحات / عقارات سياحية"
{
  "label": "تأجير شاليهات",
  "emoji": "🏖️",
  "accent": "#0891b2",
  "tagline": "أدِر حجوزات وحداتك بسلاسة",
  "terms": {
    "item": "شاليه",
    "items": "الوحدات",
    "itemAdd": "➕ إضافة وحدة",
    "catalog": "🏠 الوحدات المتاحة",
    "order": "حجز",
    "orders": "الحجوزات",
    "orderInbox": "🔑 الحجوزات الواردة",
    "customer": "ضيف",
    "cart": "📋 حجزك",
    "delivery": "استلام المفاتيح"
  },
  "fields": { "hasStock": true, "hasSize": false, "hasDuration": true, "hasHourly": false },
  "tabs": ["bookings","menu","customers","loyalty","accounting","settings","whatsapp"],
  "features": ["appointBook","gallery","reviews","invoices"],
  "dashboardCards": [
    {"key":"occupancyRate","title":"معدل الإشغال","emoji":"📊","metric":"percent","color":"#0891b2"},
    {"key":"todayCheckins","title":"check-in اليوم","emoji":"🔑","metric":"count","color":"#10b981"},
    {"key":"todayCheckouts","title":"check-out اليوم","emoji":"👋","metric":"count","color":"#f59e0b"},
    {"key":"monthRevenue","title":"إيرادات الشهر","emoji":"💰","metric":"sar","color":"#16a34a"}
  ],
  "quickActions": [
    {"label":"حجز يدوي","emoji":"➕","action":"addItem","target":"bookings"},
    {"label":"وحدة جديدة","emoji":"🏠","action":"addItem","target":"menu"},
    {"label":"تقويم الإشغال","emoji":"📅","action":"openTab","target":"bookings"}
  ],
  "itemFieldsExtra": [
    {"key":"bedrooms","label":"عدد غرف النوم","type":"number"},
    {"key":"bathrooms","label":"عدد الحمامات","type":"number"},
    {"key":"maxGuests","label":"الضيوف الأقصى","type":"number"},
    {"key":"sizeM2","label":"المساحة (م²)","type":"number"},
    {"key":"location","label":"الموقع (حي/مدينة)","type":"text"},
    {"key":"checkInTime","label":"وقت الـ check-in","type":"text"},
    {"key":"checkOutTime","label":"وقت الـ check-out","type":"text"},
    {"key":"minNights","label":"أقل عدد ليالي","type":"number"},
    {"key":"cancellationPolicy","label":"سياسة الإلغاء","type":"select","options":["flexible","moderate","strict"]}
  ],
  "emptyStates": {
    "menu":"لا توجد وحدات بعد — أضف شاليهك/منزلك الأول مع الصور والمرافق",
    "orders":"لا توجد حجوزات نشطة — ستظهر هنا فور حجز الضيوف",
    "customers":"لا ضيوف بعد — كل ضيف يحجز سيظهر هنا تلقائياً"
  },
  "tips": [
    "أضف صور احترافية لكل وحدة (الواجهة + الغرف + المسبح) — تزيد الحجوزات 3×",
    "حدد سعر/ليلة وأقل عدد ليالي لكل وحدة (مثلاً ليلتان حد أدنى في الويكند)",
    "فعّل الواي فاي + المسبح + الشواء كأبرز المرافق في قائمة الوحدة"
  ],
  "orderStatusFlow": ["حجز جديد","مؤكد","check-in","check-out","مكتمل"],
  "orderMode": "booking",
  "orderModeReason": "تأجير العقارات يتطلب فترة زمنية (check-in → check-out) لوحدة محددة، لا منتجات بكميات",
  "completionLabel": "تم check-out",
  "completionEmoji": "🔑",
  "hasInventory": true,
  "menuLayout": {
    "showQuantityButtons": false,
    "showCartIcon": false,
    "askForDeliveryFee": false,
    "primaryButtonText": "احجز هذه الوحدة"
  },
  "settings": {
    "welcomeMessage": "🏖️ أهلاً بك في {{store_name}}\n\nاختر وحدتك المفضلة وحدد تاريخ الوصول والمغادرة، وسنؤكد الحجز خلال دقائق.\n\nاكتب *الوحدات* لرؤية القائمة 🏠",
    "businessHours": { "start": 0, "end": 24, "note": "الحجوزات متاحة 24/7 — لكن check-in/out في أوقات محددة لكل وحدة" },
    "deliveryFeeRecommended": 0,
    "policies": [
      "يلزم دفع 50% عربون لتأكيد الحجز",
      "إلغاء قبل 48ساعة من الوصول → استرداد كامل",
      "عدد الضيوف لا يتجاوز الحد المعلن في كل وحدة"
    ],
    "settingsTips": [
      "💡 ضع أوقات check-in/out واضحة في وصف كل وحدة",
      "💡 فعّل تنبيهات SMS قبل check-in بـ24ساعة"
    ]
  }
}

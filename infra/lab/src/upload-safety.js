...
// 📖 صيغ المنيو الجاهز = صور + PDF
const MENU_EXTS  = new Set([...IMAGE_EXTS, ...DOC_EXTS]);
 ...
 */
function decodeAndVerifyBase64(base64, ext, maxBytes, kind /* "image" | "video" | "menu" */) {
  if (!base64 || typeof base64 !== "string") return { ok: false, error: "بيانات مفقودة" };
  const cleaned = base64.replace(/^data:(image|video|application)\/\w[\w+.-]*;base64,/, "");
  let buffer;
  ...
  const allowed = kind === "video" ? VIDEO_EXTS
                : kind === "menu"  ? MENU_EXTS
                :                    IMAGE_EXTS;
